...
} elseIF ($paket!=="UPGRADE-PLATINUM") {
ob_start();
header("Location: index.php?error=GAGAL - GUNAKAN PIN UPGRADE PLATINUM");
} elseIF ($status=='0') {
ob_start();
header("Location: index.php?error=GAGAL - PIN TIDAK AKTIF");
} elseIF ($used=='1') {
ob_start();
header("Location: index.php?error=PIN SUDAH DIGUNAKAN");
} else {

$update = mysqli_query($koneksi, "UPDATE mebers SET paket='PLATINUM', tglupgrade=now() WHERE id='$id' ")or die(mysql_error());
$update2 = mysqli_query($koneksi, "UPDATE pin SET used='1', userid='$row[userid]', tanggal=now() WHERE pin='$pinupgrade' ")or die(mysql_error());

if($paket_member=='YUNIOR BRONZE') {$bonus_upgrade=1750000;}
if($paket_member=='BRONZE') {$bonus_upgrade=1750000;}
if($paket_member=='SILVER') {$bonus_upgrade=1500000;}
if($paket_member=='GOLD') {$bonus_upgrade=1000000;}

$insert= mysqli_query($koneksi, "INSERT INTO bonus_sponsor SET userid='$row[sponsor]', bonusfrom='$row[userid]', bonus='$bonus_upgrade', paket='$paket_member TO PLATINUM', timer=now()")or die(mysql_error());
...
?>
<head>
  <title>Upgrade Platinum | Tombo Ati</title>
</head>
...
if ($paket=='YUNIOR PRONZE' OR $paket=='BRONZE' OR $paket=='SILVER' OR $paket=='GOLD'){
    $tampil='
...
<a href="#" class="btn btn-sm btn-SUCCESS"> <font size="3">MEMBERSHIP <?php echo $paket;?> <?php echo $id;?></font> <i class="fa fa-arrow-circle-right"></i></a><br><br>


                    <tr>
<td> 
<?php echo $tampil;?>                                 
<a href="#" class="btn btn-sm btn-warning">  Masukkan PIN Upgrade PLATINUM  </i></a>                                 
</td>
<br><br>
                    </tr>
                  </thead>
