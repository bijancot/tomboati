...
} elseIF ($paket!=="UPGRADE-GOLD") {
ob_start();
header("Location: index.php?error=GAGAL - GUNAKAN PIN UPGRADE GOLD");
} elseIF ($status=='0') {
ob_start();
...
$update = mysqli_query($koneksi, "UPDATE mebers SET paket='GOLD', tglupgrade=now() WHERE id='$id' ")or die(mysql_error());
$update2 = mysqli_query($koneksi, "UPDATE pin SET used='1', userid='$row[userid]', tanggal=now() WHERE pin='$pinupgrade' ")or die(mysql_error());

if($paket_member=='YUNIOR BRONZE') {$bonus_upgrade=750000;}
if($paket_member=='BRONZE') {$bonus_upgrade=750000;}
if($paket_member=='SILVER') {$bonus_upgrade=500000;}

$insert = mysqli_query($koneksi, "INSERT INTO bonus_sponsor (userid, bonusfrom, paket, bonus, timer) VALUES ('$row[sponsor]','$row[userid]','$paket_member TO GOLD',$bonus_upgrade,now()) ") or die(mysqli_error());
...
?>
<head>
  <title>Upgrade Gold | Tombo Ati</title>
</head>
...
if ($paket=='YUNIOR PRONZE' OR $paket=='BRONZE' OR $paket=='SILVER'){
    $tampil='
...
<td> 
<?php echo $tampil;?>                                 
<a href="#" class="btn btn-sm btn-warning">  Masukkan PIN Upgrade GOLD  </i></a>                                 
</td>
<br><br>
                    </tr>
                  </thead>
