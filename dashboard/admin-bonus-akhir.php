...
?>
<head>
  <title>Bonus Akhir | Tombo Ati</title>
</head>
...
if(isset($_GET['halaman']))
{
    ...
    $query1 = "SELECT * FROM mebers WHERE paket='RESELLER' ORDER BY id DESC limit $default_index, $default_batas";
...
$tampil_nama=mysqli_query($koneksi, "select * from mebers WHERE userid='$data[sponsor]' ");
$data_nama=mysqli_fetch_array($tampil_nama);
?>
                    <tbody>
                    <tr>
                    <td><left><?php echo $no; ?>.</left></td>
                    <td align="left"><?php echo $data['sponsor'];?></td>
                    <td><left><?php echo $data_nama['name'];?></left></td>
                    <td align="left"><?php echo $data_nama['paket'];?></td>
                    <td><center><?php echo $data['timer'];?></center></td>
                    <td><left><?php echo $data['userid'];?></left></td>
                    <td align='right'><?php echo number_format(1100000,0,",",".");?></td>
